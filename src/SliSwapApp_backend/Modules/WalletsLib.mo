...
module {
    ...
    public func AddNewApprovedWallet(
        caller : Principal,
        ...
        if (List.size(approvedWallets.approvedWalletsFree) > 0) {
            ...
            let containsInFreeList = List.some<Principal>(
                approvedWallets.approvedWalletsFree,
                listFindFunc,
            );
            ...
            let containsInUseList = List.some<Principal>(
                approvedWallets.approvedWalletsInUse,
                listFindFunc,
            );
    ...
    public func ApprovedWalletsPrincipalExist(
        principal : Principal,
        approvedWalletsSli : T.ApprovedWallets,
        approvedWalletsGlds : T.ApprovedWallets,
    ) : Bool {

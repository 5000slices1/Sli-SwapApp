...
module {
    ...
    public func getSwapWallet(
        caller : Principal,
        usersSwapInfo : T.UsersSwapInfo,
    ) : T.ResponseGetUsersSwapWallet {
    ...
    private func getUserSwapInfoItem(
        principal : Principal,
        usersSwapInfo : T.UsersSwapInfo,
    ) : Result.Result<T.UserSwapInfoItem, Text> {
    ...
    public func SomeConversionIcrc1StillOnProgress(principal : Principal, dataPerToken : T.CommonDataPerToken) : Bool {
        ...
        let conversionStartedStateInfo = GetProgressStartedTime(
            encodedPrincipal,
    ...
    public func GetDip20DepositedAmount(
        usersPrincipal : Principal,
        dip20CanisterId : Text,
        usersSwapInfo : T.UsersSwapInfo,
        transferFee : Nat,
    ) : async* Result.Result<(Nat, Nat, Principal), Text> {
    ...
    public func DepositDip20Tokens(
        usersPrincipal : Principal,
        dip20CanisterId : Text,
        swapAppPrincipal : Principal,
        dataPerToken : T.CommonDataPerToken,
        amount : Nat,
        fee : Nat,
        ...
        switch (swapWalletResponse) {
            ...
            case (#NotExist) {
                ...
                dataPerToken.approvedWallets.approvedWalletsInUse := List.push(
                    swapWalletPrincipal,
    ...
    private func PrepareAndTransferTheIcrc1TokensIntoSwapAppSubAccount(
        usersPrincipal : Principal,
        dataPerToken : T.CommonDataPerToken,
        icrcCanisterId : Text,
        dip20CanisterId : Text,
        ...
        switch (conversionStartedStateInfo) {
            case (#ok(startedTime)) {
                ...
                let transferToSubAccountState = GetProgressStartedTime(
                    encodedPrincipal,
    ...
    private func TransferIcrc1ToUserWalletAndBurnOldDip20(
        usersPrincipal : Principal,
        dataPerToken : T.CommonDataPerToken,
        icrcCanisterId : Text,
        ...
        let previousStepWasStarted = IsProgressStateSet(
            GetProgressStartedTime(
                encodedPrincipal,
                dataPerToken.convertState.transferToSubaccountStarted,
            )
        );
                ...
                case (#Ok(txIndex)) {
                    ...
                    switch (swapInfoItemResponse) {
                        case (#ok(swapInfoItem)) {
                            let newSwapInfoItem = {
                                swapInfoItem with depositCount = 0
                            };
                            StableTrieMap.put(
                                dataPerToken.swapInfo.userSwapInfoItems,
                                Blob.equal,
                                Blob.hash,
                                encodedPrincipal,
                                newSwapInfoItem,
    ...
    public func ConvertOldDip20Tokens(
        userId : Blob,
        dataPerToken : T.CommonDataPerToken,
        dip20CanisterId : Text,
        dip20TransferFee : Nat,
        ...
        let encodedPrincipal : Blob = Principal.toBlob(usersPrincipal);
        let secondStepAlreadyStarted = IsProgressStateSet(
            GetProgressStartedTime(
                encodedPrincipal,
        ...
        if (secondStepAlreadyStarted == false) {
            ...
            response := await* PrepareAndTransferTheIcrc1TokensIntoSwapAppSubAccount(
                usersPrincipal,
                dataPerToken,
                icrcCanisterId,
                dip20CanisterId,
        ...
        //Step 2
        response := await* TransferIcrc1ToUserWalletAndBurnOldDip20(
            usersPrincipal,
            dataPerToken,
            icrcCanisterId,
            appPrincipal,
    ...
    private func GetPrincipalFromBlob(
        userId : Blob,
        usersSwapInfo : T.UsersSwapInfo,
    ) : Result.Result<Principal, Text> {
    ...
    public func BlobToText(blob : Blob) : Text {
        ...
        for (byte : Nat8 in blob.vals()) {
            // iterate over the Blob
